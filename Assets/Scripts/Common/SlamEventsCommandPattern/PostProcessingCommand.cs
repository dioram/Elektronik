...
namespace Elektronik.Common.SlamEventsCommandPattern
{
    public class PostProcessingCommand : MacroCommand
    {
        public PostProcessingCommand(SlamPointsContainer pointsContainer, SlamLinesContainer linesContainer, ISlamEvent slamEvent)
        {
            ...
            {
                SlamPoint[] points = GetPoints(slamEvent.Points.Where(p => p.id != -1).ToArray());
                m_commands.Add(new RepaintPointsCommand(pointsContainer, points));
            }
            if (slamEvent.Lines != null)
            {
                SlamLine[] lines = GetLines(slamEvent.Lines);
                m_commands.Add(new RepaintLinesCommand(pointsContainer, linesContainer, lines));
            }
        }
