...
Либо вы можете собрать проигрыватель через интерфейс редактора Unity. В этом случае укажите для сборки 
проекта директорию `./build`, иначе для плагинов придётся поменять путь к зависимостям на выбранный вами.
